"""
Uses Stable-Baselines3 to train agents in the Legacy's Allure environment using invalid action masking.
...
class SB3ActionMaskWrapper(pettingzoo.utils.BaseWrapper):
    """
    Wrapper to allow PettingZoo environments
    to be used with SB3 illegal action masking.
    """

    def reset(self, seed=None, options=None):
        """
        Gymnasium-like reset function which assigns
        obs/action spaces to be the same for each agent.

        This is required as SB3 is designed for single-agent RL
        and doesn't expect obs/action spaces to be functions
        """
    ...
    def step(self, action):
        """
        Gymnasium-like step function, 
        returning observation, reward, termination, truncation, info.
        """
        super().step(action)
        return super().last()
...
if __name__ == "__main__":
    ...
    eval_action_mask(env_fn, num_games=5, human_readable=True, vs_human=True)